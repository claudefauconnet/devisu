...
points.append("text").attr("x", function(d) {
	...
	dragRect = svgRadar.selectAll().data(dataRect).enter().append("rect").attr("width", function(d) {
		...
		return d.y;
	}).style("z-index", 100).style("stroke", "black").style("fill", "transparent").attr("class", "dragRect").style("visibility", "hidden");
	...
	var dragPoints = d3.behavior
			...
			.on("drag", function(d, sx, sa, sy, sz) {
				...
				if (isResizing) {
					...
					d3.select(this).datum().w=newWidth;
					d3.select(this).datum().h=newHeight;
					d3.select(this).attr("width", newWidth).attr("height", newHeight);
					}
			...
			.on(
					...
					function(d, sx, sa, sy, sz) {
						...
						//	 shape.attr("transform", "translate(" + x + "," + y + ")," + "scale(" + coefX + "," + coefY + ")");
