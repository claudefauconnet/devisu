...
function drawLegend(svgGroup) {
	...
	function clickLegend() {
		...
		if (e.ctrlKey) {
			...
			if (xx > 0)
				opacity = 0.3;
			var xxxxx = d3.select(this).select(".legendText")
			var xxxxx = d3.select(this).select(".legendText").style("opacity",
					0.2);
...
// drag.origin(vis);
	...
	drag.on("drag", function(d) {
		...
		d.y +=d3.event.dy;
			...
			d3.select(this).attr("transform", function(d) {
				return "translate(" + d3.event.dx+ "," +  d3.evemoulinnt.dy+ ")";
			});
	...
	drag.on("dragend", function(d) {
		...
		d3.select(this).attr("transform", function(d) {
		 ...
		 /*
			 * d.x = d3.event.sourceEvent.offsetX; d.y
			 * =d3.event.sourceEvent.offsetY; console.log(d.x+" "+ d.y)
			 * d3.select(this).attr("x",d.x).attr("y", d.y);
			 * d3.event.sourceEvent.stopPropagation(); // silence other
			 * listeners
			 */
...
function drawForceCollapse(json, _w, _h, _charge, _distance) {
	...
	var w = 1280, h = 800, distance = 120, charge = -300;
	var node, link, root;
	...
	function update() {
		...
		nodeEnter = nodeEnter.enter().append("g").attr("class", "node").attr(
				"transform", function(d) {
					return "translate(" + d.x + "," + d.y + ")";
				}).on("click", clickThis).attr("pointer-events", "mouseover")
		...
		nodeEnter.append("circle").attr('class', 'nodeCircle').attr("r",
				function(d) {
					...
					if (d.isRoot === true)
						return circleR;
					return circleR / Math.pow(2, d.level + 1) * 3;
		...
		nodeEnter.append("text").attr("x", function(d) {
			// return d.x < rootx ? -8 : circleR + 8;
			var xx;
			if (d.isRoot === true)
				xx = circleR;
			else
				xx = (circleR / Math.pow(2, d.level + 1) * 3);
		...
		.style("fill", function(d) {
			...
			if (d.name.length > nodeTextLength)
				return d.name.substring(0, nodeTextLength - 1) + "...";
			return d.name;
		}).style("fill-opacity", 1).style("font-size", "12px").style("fill",
	...
	function overCircle(node) {
		hoverShow(node.x, node.y, "[" + node.type + "] " + node.name);
	...
	function clickThis() {
		 ...
		 */
			...
			/*
			 * query2("MATCH (n:" + selectedNode.type + "{id:" +
 ...
 * 
 * }).style("fill", function(d) { var color = nodeColors[d.type]; return color; }) ;
 * 
...
function clickX() {
	...
	if (e.ctrlKey) {
		 ...
		 * selectedNode.color=nodeColors[selectedNode.type];
